    ...
    <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
        ...
        <div className="border-b">
          ...
          >
            <span className="flex items-center gap-1.5">
              <Wifi className="h-3.5 w-3.5 text-orange-500" aria-hidden="true" />
              Redes Wi-Fi
      ...
      {data && data.clients.length > 0 && (
        <div>
          ...
          {clientsOpen && (
            <div className="overflow-x-auto border-t">
              <table className="w-full text-xs" aria-label="Clientes Omada conectados">
                <thead className="bg-muted/40">
                  <tr>
...
export default function OmadaPage() {
  ...
  async function handleCheckAll() {
    setIsChecking(true);
    await fetch("/api/devices/check?type=OMADA_AP", { method: "POST" }).catch(() => {});
    await load();
    setIsChecking(false);
  }
